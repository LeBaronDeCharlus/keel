...
fn ensure_mount_point_creates_missing_parent_directories() {
    ...
    mounts
        ...
        .expect("ensure_mount_point should succeed");
...
fn mount_nullfs_then_is_mounted_then_unmount_round_trips_through_the_real_kernel() {
    ...
    assert_eq!(
        mounts.is_mounted(target).unwrap(),
        false,
        "must not be mounted before mount_nullfs"
    );

    mounts
        .mount_nullfs(source, target)
        .expect("mount_nullfs should succeed");
...
fn mount_nullfs_called_twice_in_a_row_does_not_stack_a_duplicate_mount() {
    ...
    mounts
        .mount_nullfs(source, target)
        .expect("first mount_nullfs should succeed");
    mounts
        .mount_nullfs(source, target)
        .expect("second mount_nullfs should succeed, not stack a duplicate");
    ...
    let entries = String::from_utf8_lossy(&output.stdout)
        .lines()
        .filter(|line| line.contains(target_str.as_ref()))
        .count();
    assert_eq!(
        entries, 1,
        "expected exactly one mount(8) entry for {target_str}, got {entries}"
    );
    ...
    assert_eq!(
        mounts.is_mounted(target).unwrap(),
        false,
        "one unmount must fully clear a mount_nullfs that was called twice"
    );
}
